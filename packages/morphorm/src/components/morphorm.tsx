...
const getDefaultValues = (parsedFields: InternalField[]) => parsedFields.reduce(
    (acc, field) => {
...
const ContextAwareField = ({ col, mode }: ContextAwareFieldProps) => {
  ...
  const slicedContext = useMemo(() => {
    ...
    const newResult = col.watchContext.reduce((acc, key) => ({ ...acc, [key]: context?.[key] }), {} as any);
    ...
    return (
      <div className="forma-group">
        <Collapsible defaultOpen>
          <form.Field name={col.name as never} mode="array">
            {(field: any) => {
              const items = (field.state.value as unknown[]) || [];

              return (
                <div className="forma-flex forma-flex--col forma-flex--gap-2">
                  <CollapsibleTrigger className="forma-bg-background" asChild>
                    <div>
                      <ArrayItemControl
                    ...
                    {items.length === 0 ? (
                      ...
                      <div className="forma-flex forma-flex--col forma-flex--gap-3">
                        {items.map((_, idx) => {
                          ...
                          return (
                            <div className="forma-array-item" key={idx}>
                              <Button
                                type="button"
          ...
          const renderGrid = (
            <div className="forma-row">
              {row.map((col) => (
                ...
                >
                  {col.type !== "hidden" && <ContextAwareField col={col} mode={col.mode} />}
                </div>
              ))}
            </div>
          );
    ...
    <FormComponentsProvider value={{ components, context, schema: parsedFields }}>
      <form.AppForm>
        <Form className="forma-form forma-form--gap-6">
          ...
          />
          {showSubmit && <SubmitButton handleCancel={handleCancel} {...buttonSettings} />}
          {children}
        </Form>
      </form.AppForm>
